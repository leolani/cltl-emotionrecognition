...
from cltl.emotion_extraction.default_extractor import DefaultEmotionExtractor
...
class EmotionExtractionContainer(InfraContainer):
    @property
    @singleton
    def emotion_extractor(self) -> EmotionExtractor:
        return DefaultEmotionExtractor()

    @property
    @singleton
    def emotion_extraction_service(self) -> EmotionExtractionService:
        return EmotionExtractionService.from_config(self.emotion_extractor,
                                                    self.event_bus, self.resource_manager, self.config_manager)

    def start(self):
        logger.info("Start Emotion Extraction Service")
        super().start()
        self.emotion_extraction_service.start()

    def stop(self):
        logger.info("Stop Emotion Extraction Service")
        self.emotion_extraction_service.stop()
        super().stop()

class ApplicationContainer(NLPContainer, EmotionExtractionContainer):
    ...
    @singleton
    def event_bus(self):
        config = self.config_manager.get_config("cltl.emotion-detection.events")
